...
function detectStructuredGesture(hand) {
  ...
  // 核心：忽略小指干扰，只看食指/中指/无名指组合
  if (indexUp && !middleUp && !ringUp) return "chew";
  if (indexUp && middleUp && !ringUp) return "suck";
  if (indexUp && middleUp && ringUp) return "fungus";
...
function handleStructuredGesture(gestureName) {
  ...
  if (pendingGestureFrames < GESTURE_HOLD_FRAMES) return;

  const now = millis();
  if (now - lastGestureSwitchTime < GESTURE_SWITCH_COOLDOWN) return;

  currentFingerGesture = gestureName;
...
function onHandResults(results) {
  if (results && results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
    ...
    const detected = detectStructuredGesture(hand);
    handleStructuredGesture(detected);

    const moved = dist(handPoint.x, handPoint.y, prevHandPoint.x, prevHandPoint.y);

    if (detected !== "explore" && moved > 6) {
